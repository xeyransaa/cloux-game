...
const Games = () => {
 ...
 const [displayedGames, setDisplayedGames] = useState([])
  const getGamesByPage = (pageNumber, gamesPerPage) => {
    fetch(BASE_URL + `Game/page/?pageNumber=${pageNumber}&gamesPerPage=${gamesPerPage}`)
    .then((c) => c.json())
...
<div className="grid grid-cols-2 md:grid-cols-3 gap-8">
        ...
        {
          games.length > gamesPerPage && <ul className='unstyled flex flex-row flex-wrap items-center justify-center mt-[50px] gap-y-[20px]'>
          ...
          {
            pageNumber<=3 ? (
              Array.from({ length: pagesCount > 5 ? 5 : pagesCount }).map((_, i) => (
              
                <li className='relative' key={i} onClick={() => {
                  changePage(i+1)
                  
                  
                }}>
                  <PlatformButton name={i+1} isActive={pageNumber === i+1}/>
              ...
              Array.from({ length: pagesCount > 5 ? 5 : pagesCount }).map((_, i) => (
                ...
                <li className='relative' key={i} onClick={() => {
                  ...
                  <PlatformButton name={i+pagesCount - 4} isActive={pageNumber === i+pagesCount - 4}/>
              ...
              Array.from({ length: pagesCount > 5 ? 5 : pagesCount }).map((_, i) => (
                ...
                <li className='relative' key={i} onClick={() => {
                  ...
                  <PlatformButton name={i+pagesCount-2} isActive={pageNumber === i+ pagesCount-2}/>
